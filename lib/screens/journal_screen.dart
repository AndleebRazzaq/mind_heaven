...
class _JournalScreenState extends State<JournalScreen> {
  ...
  static const Map<String, String> _plantMoodLineByName = {
    'lavender': 'Lavender is often associated with relaxation and gentle focus.',
    'snake plant': 'Snake Plant can make your space feel steadier and grounded.',
  ...
  Future<void> _onAnalyze() async {
    ...
    if (text.isEmpty) {
      setState(() {
        _inputError = 'Share a few lines first so I can support you meaningfully.';
      });
      return;
    }
    if (text.length < 20) {
      setState(() {
        _inputError = 'Write at least 20 characters so the reflection can be more accurate.';
      });
      return;
    }
    setState(() => _inputError = null);

    await context.read<JournalProvider>().analyze(
      text,
      stressBefore: null,
    );
    if (!mounted) return;
  ...
  String _resolvedTone(CBTIntervention i) {
    ...
    if (incoming == 'gentle' || incoming == 'balanced' || incoming == 'direct') {
      return incoming;
    }
  ...
  String _distortionInsight(CBTIntervention i) {
    final line = i.distortionLogicLine?.trim() ?? i.distortionInsightLine?.trim();
  ...
  Widget _buildStepCard({required Key key, required CBTIntervention intervention}) {
    ...
    switch (step) {
      case _OutputStep.emotion:
        final emotionLabel = intervention.moodLabel?.trim().isNotEmpty == true
            ...
            : (intervention.emotionConfidence != null ? 'Detected' : 'Not available');
        return _StepCard(
          key: key,
          ...
          body: Column(
            ...
            children: [
              ...
              if (intervention.emotionalSupportMessage?.trim().isNotEmpty ?? false) ...[
                const SizedBox(height: 8),
                Container(
      ...
      case _OutputStep.pattern:
        final pattern = intervention.detectedDistortionLabel?.trim().isNotEmpty == true
...
class _BreathingExerciseCardState extends State<_BreathingExerciseCard>
  ...
  void _animateForPhase(String phase) {
    if (phase == 'Inhale') {
      _controller.animateTo(1.2, duration: const Duration(seconds: _inhaleSeconds));
    } else if (phase == 'Exhale') {
      _controller.animateTo(0.75, duration: const Duration(seconds: _exhaleSeconds));
    } else {
      _controller.stop();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(widget.techniqueTitle ?? 'Take a 60-second pause to calm your body.'),
        const SizedBox(height: 6),
        Text(
            ...
            builder: (_, __) => Transform.scale(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF60A5FA).withOpacity(0.75), width: 2),
                ),
                child: Center(
                  child: Text(
